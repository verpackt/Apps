...
st.set_page_config(page_title="Resumen de Libros en PDF (Capítulos limpios)", layout="wide")
...
# Función para dividir texto por capítulos en español e inglés
def dividir_por_capitulos(texto):
    texto = texto.replace("\n", "
")
    texto_normalizado = texto.lower()
    capitulos = re.split(r'(cap[ií]tulo\s+\d+|chapter\s+\d+)', texto_normalizado, flags=re.IGNORECASE)
    estructura = []
    for i in range(1, len(capitulos), 2):
        titulo = capitulos[i].strip().capitalize()
